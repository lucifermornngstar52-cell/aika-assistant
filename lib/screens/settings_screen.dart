...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _save() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('assistant_name', _nameController.text.trim().isEmpty ? 'Aika' : _nameController.text.trim());
  ...
  Widget _section(String title) => Padding(
    ...
    child: Text(title, style: TextStyle(color: AikaTheme.neonBlue, fontSize: 12, letterSpacing: 2, fontWeight: FontWeight.bold)),
  );
  ...
  Widget _slider(String label, double value, double min, double max, ValueChanged<double> onChanged) => Column(
    ...
    children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        Text(value.toStringAsFixed(2), style: TextStyle(color: AikaTheme.neonBlue, fontSize: 12)),
      ]),
      SliderTheme(
      ...
      body: _loading
          ...
          : ListView(
              ...
              children: [
                ...
                _section('ГОЛОС'),
                _slider('Скорость речи', _ttsRate, 0.1, 1.0, (v) => setState(() => _ttsRate = v)),
                _slider('Тон голоса', _ttsPitch, 0.5, 2.0, (v) => setState(() => _ttsPitch = v)),
                _slider('Громкость', _ttsVolume, 0.0, 1.0, (v) => setState(() => _ttsVolume = v)),
