...
class ElevenLabsTtsService {
  ...
  static const String _envApiKey = String.fromEnvironment('ELEVENLABS_API_KEY', defaultValue: '');
  // ФИКС: ключ можно вписать прямо в настройках голоса (prefs) —
  // тогда смена ключа не требует пересборки APK
  String _prefsApiKey = '';
  Future<String> _getApiKey() async {
    if (_prefsApiKey.isEmpty) {
      try {
        final prefs = await SharedPreferences.getInstance();
        _prefsApiKey = prefs.getString('elevenlabs_key') ?? '';
      } catch (_) {}
    }
    return _prefsApiKey.isNotEmpty ? _prefsApiKey : _envApiKey;
  }
  ...
  bool get isAvailable => _envApiKey.isNotEmpty;
  ...
  Future<void> initialize() async {
    ...
    await _getApiKey();
    if (_envApiKey.isEmpty && _prefsApiKey.isEmpty) {
  ...
  Future<String?> _synthesize(String text) async {
    ...
    if (apiKey.isEmpty || _selectedVoiceId.isEmpty) return null;
